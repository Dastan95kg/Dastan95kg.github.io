...
async function getTranslation(text) {
    ...
    if (response.status === 200) {
        ...
        if (text === data[0].text.toLowerCase()) {
            ...
            audio.className = 'translation__audio';
            audio.innerHTML = `<audio src="${soundUrl}" controls autoplay></audio>`;
            resultWrapper.appendChild(audio);

            // add image
            const imageUrl = data[0].meanings[0].imageUrl;
            const image = document.createElement('div');
            image.className = 'translation__image-wrapper';
            image.innerHTML = `<img class="translation__image" src="${imageUrl}" />`;
